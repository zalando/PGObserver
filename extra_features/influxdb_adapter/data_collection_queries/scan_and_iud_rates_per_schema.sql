...
select
  *,
  lag(tsd_timestamp) over(partition by t_schema order by tsd_timestamp) as timestamp_lag,
  lag(tscans) over(partition by t_schema order by tsd_timestamp) as tscans_lag,
  lag(iscans) over(partition by t_schema order by tsd_timestamp) as iscans_lag,
  lag(ins) over(partition by t_schema order by tsd_timestamp) as ins_lag,
  lag(upd) over(partition by t_schema order by tsd_timestamp) as upd_lag,
  lag(del) over(partition by t_schema order by tsd_timestamp) as del_lag
from (
  select
    tsd_timestamp,
    t_schema,
