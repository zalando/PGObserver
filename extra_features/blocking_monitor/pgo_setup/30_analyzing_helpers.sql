...
DROP FUNCTION IF EXISTS blocking_last_day_by_shortname(text, timestamp without time zone, timestamp without time zone, boolean);

CREATE OR REPLACE FUNCTION blocking_last_day_by_shortname(
    IN p_host_ui_shortname text default null,
...
BEGIN
  ...
  RETURN QUERY
	...
	(
	 ...
	 inner join hosts on host_id = blocked.bl_host_id and (hosts.host_ui_shortname = p_host_ui_shortname or p_host_ui_shortname is null)
	...
	select 	hosts.host_ui_shortname,
	...
	group by hosts.host_ui_shortname,
		a.blocking_query
	order by 1, 2 desc;
...
GRANT EXECUTE ON FUNCTION blocking_last_day_by_shortname(text, timestamp without time zone, timestamp without time zone, boolean) to pgobserver_frontend;
